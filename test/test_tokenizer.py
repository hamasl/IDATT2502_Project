...
class TokenizerTest(unittest.TestCase):
	...
	def test_tokenize_lines(self):
		...
		tokenizer = tn.Tokenizer(2)
		self.assertEqual(tokenizer.file_tokenize(input), expected)

	def test_remove_comments(self):
		input = ["/*int i = 0;*/\n if(1) return 1;\n"]
		expected = [['if', '(', '1', ')', 'return', '1', ';']]
		tokenizer = tn.Tokenizer(1)
		self.assertEqual(tokenizer.file_tokenize(input), expected)
	...
	def test_read_functions_from_file(self):
		...
		tokenizer = tn.Tokenizer(1)
		functions, types, _ = tokenizer.get_functions(os.path.join(dirname,"test_data/CWE835_Infinite_Loop__for_01.c"), 1, 1)
